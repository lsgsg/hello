...
$del_file[]=$_POST['del_file[]'];
	...
	if ($mode=="modify")
	{
		...
		$sql = "SELECT * FROM concert WHERE num=$_GET[num]";   // get target record
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y")
			{
				...
				$sql = "update concert set $field_org_name = '$org_name_value', $field_real_name = '$org_real_value'  where num=$_GET[num]";
		...
		$sql = "update concert set subject='$subject', content='$content' where num=$_GET[num]";
		mysql_query($sql, $connect);  // $sql 에 저장된 명령 실행
	}
	else
	{
