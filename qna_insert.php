...
$content=$_POST[content];
	...
	if ($_GET[mode]=="modify")
	{
		$sql = "update $_GET[table] set subject='$subject', content='$content' where num=$_GET[num]";
	...
	else
	{
		...
		if ($_GET[mode]=="response")
		{
			...
			// ord 값 1 증가 시킴
			$sql = "update $_GET[table] set ord = ord + 1 where group_num = $row[group_num] and ord > $row[ord]";
			mysql_query($sql, $connect);  

			// 레코드 삽입
			$sql = "insert into $_GET[table] (group_num, depth, ord, id, name, nick, subject,";
		...
		else
		{
			...
			// 레코드 삽입(group_num 제외)
			$sql = "insert into $_GET[table] (depth, ord, id, name, nick, subject,";
			$sql .= "content, regist_day, hit, is_html) ";
			...
			// group_num 값 업데이트 
			$sql = "update $_GET[table] set group_num = $auto_num where num=$auto_num";
			mysql_query($sql, $connect);
		}
	}
	...
	echo "
	   <script>
	    location.href = 'qna_list.php?table=$_GET[table]&page=$_GET[page]';
	   </script>
	";
?>